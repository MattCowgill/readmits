tidy_mits <- function(path) {
  ...
  tidier_fn <- switch(substr(tolower(base_file), 1, 5),
                      ...
                      stop("Currently unable to tidy files like ",
                           path))
...
tidy_mits_wages <- function(path) {
  ...
  names(raw_wages)[2:8] <- paste0(c(rep("Expected Wage Growth: ", 3),
  ...
  raw_wages %>%
    ...
    tidyr::pivot_longer(cols = !dplyr::one_of("date"),
                        names_to = "series") %>%
    dplyr::mutate(table = "Wages growth",
                  value = as.numeric(.data$value)) %>%
    dplyr::filter(!is.na(.data$value))
}
